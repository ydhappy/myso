...
--
-- 목적:
--   기존 구버전 에고 DB를 현재 병합 DB 구조로 한번에 보정합니다.
--
-- 실행 순서:
...
--   3) ego_merge_schema_euckr.sql   ego_level 통합 + ego.bond 컬럼 병합
--   4) ego_talk_pack_dedupe.sql     대사팩 중복 정리
--
-- 주의:
--   SOURCE 문은 mysql CLI, HeidiSQL, Navicat 등에서 동작 방식이 다를 수 있습니다.
--   SOURCE가 안 되는 툴이면 아래 4개 파일을 순서대로 직접 실행하세요.
-- ============================================================
...
SELECT 'EGO_UPDATE_EUCKR_MERGED_SCHEMA_ALL_OK' AS result;
